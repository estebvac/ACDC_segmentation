...
class MRI_DataPatchLoader(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        return torch.from_numpy(input_train.copy()), torch.from_numpy(input_label.copy())
    ...
    def generate_patch_indexes(self, roi_scans):
        ...
        for scan, label, roi, index in zip(self.input_scans,
                              self.label_scans,
                              roi_scans,
                              range(len(self.input_scans))):
            # sample candidates
            candidate_voxels = self.get_candidate_voxels(scan[0], label[0], roi[0])
            voxel_coords = get_voxel_coordenates(scan[0],
                                                 candidate_voxels,
                                                 step_size=self.sampling_step,
                                                 random_pad=self.random_pad)
            training_indexes += [(index, tuple(v)) for v in voxel_coords]
    ...
    def balance_csf(self, resample_csf):
        n_samples = len(self.patch_indexes)
        self.original_samples = n_samples
        added = 0
        item_to_add = []
        for counter in range(n_samples):
            patch_val, patch_label = self.__getitem__(counter)
            is_csf = np.sum(1 * (patch_label == 1)) > 0
            if is_csf:
                added += 1
                item_to_add.append(counter)

        for i in range(resample_csf):
            for counter in item_to_add:
                item = self.patch_indexes[counter]
                self.patch_indexes.append(item)

        print('Original samples:  ', n_samples,
              ', Resampled  : ', len(self.patch_indexes) )
